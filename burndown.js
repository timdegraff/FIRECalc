...
export const burndown = {
    ...
    run: () => {
        ...
        for (let j = 0; j < 15; j++) {
            ...
            const isTestSolvent = !testRes.some(r => r.status === 'INSOLVENT');
        ...
        for (let i = 0; i <= (100 - startAge); i++) {
            ...
            const performWithdrawalPass = (isSurvival = false) => {
                for (const pk of liquidOrder) {
                    ...
                    let drag = burndown.assetMeta[pk].isTaxable ? 0.22 : 0;
                    let pull = Math.min(av, gap / (1 - drag));

                    // Strategy constraints (ignored if survival pass)
            ...
            // Main Iteration Loop (10 passes for precision)
            for (let iter = 0; iter < 10; iter++) {
                ...
                // If Handout Hunter, solve for the MAGI Ceiling
                if (persona === 'PLATINUM' && config.snapPreserve > 0) {
                    ...
                    for (let j = 0; j < 20; j++) {
                        ...
                        if (testAid >= config.snapPreserve) { ceilingMAGI = mid; low = mid; } else { high = mid; }
                    }
                    
                    // Specific MAGI logic for Handout Hunter
                    ...
                    // Strategy overflow (Hit budget if gap remains)
                    performWithdrawalPass(true);
                } else {
                    performWithdrawalPass(false);
                    performWithdrawalPass(true); // Always ensure target budget is satisfied
                }
